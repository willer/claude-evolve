---
name: evolve-code
description: Write the code for one evolution candidate. Resolves the candidate's parent algorithm, copies it to evolution_<id>.py, then implements the candidate's description from evolution.csv — codex (GPT-5.6 Luna) first, with you (Fable) judging the result and coding it yourself if codex falls short. Use when the user says "code gen02-003", "write the algorithm for this candidate", "implement idea <id>", or when a parent skill dispatches coding.
...
Coding is **codex-first**: codex (GPT-5.6 Luna) takes the first pass, and you (Fable) judge whether it did the job — coding the candidate yourself when it didn't. It needs genuine coding judgment either way. (The omnibus `/evolve` loop runs this same protocol via the plugin's `claude-evolve:coder` agent, which adds claim/score looping; this skill is the standalone one-candidate path.)
...
Code the candidate. **Try codex (GPT-5.6 Luna) first; fall back to coding it yourself** only if codex falls short.
...
- **Good** (`ok:true` and your judgment agrees): codex coded it. Record the model and skip to Step 3.
  ```bash
  python3 "$CLAUDE_PLUGIN_ROOT/scripts/evolve_csv.py" --working-dir "<WORKING_DIR>" set-field <CANDIDATE_ID> run-LLM gpt-5.6-luna
  ```
...
```bash
python3 "$CLAUDE_PLUGIN_ROOT/scripts/evolve_csv.py" --working-dir "<WORKING_DIR>" set-field <CANDIDATE_ID> run-LLM fable
```
