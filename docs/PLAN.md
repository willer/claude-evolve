...
- [x] fix the CSV lock so concurrent workers stop losing scores; show kurtosis in the Greenhouse leader summary
  - Both `evolution_csv.py` copies no longer unlink `.evolution.csv.lock` on release (see TRUTH: unlinking gave each writer a fresh inode, so flock stopped excluding). Dead `lib/csv-lock.sh` removed. Plugin 0.3.2.
  - `greenhouse/src/core/profile.ts` adds `kurtosis` to TRADING_METRICS (raw Fisher excess, may be negative; absent column is skipped). 102/102 tests, typecheck clean.
- [x] rewrite README for a coworker: it's a Claude Code / Codex plugin marketplace now, not an npm package
  - README leads with the marketplace install commands, skills, model roles, workspace layout, evaluator formats, config keys the plugin reads, Greenhouse, and a short legacy-CLI section. CLAUDE.md intro updated to match.
  - Root cleanup in the same pass: stale test-*.sh, aider history, 2024 analysis docs, screenshot, sample CSVs, `test-evolution/`, `test-ideate/`, the empty root `evolution/`, and six superseded docs (PLAN-old, CLAUDE-NOTES, QUESTIONS, PARALLEL-DESIGN, GIT-WARNING-PSYCHOLOGY, IDEAS) deleted; all in git history if needed.
